...
bool PackageLoader::loadComponentFromFile(const ComponentInfo& component, const std::filesystem::path& basePath) {
    // For now, components are registered in registerBuiltinComponents
    // In the future, this could load from shared libraries
    ...
    spdlog::warn("[PackageLoader] Component {} not found in built-in registry", component.name);
    return false;
}

bool PackageLoader::loadSystemFromFile(const SystemInfo& system, const std::filesystem::path& basePath) {
    // For now, systems are registered in registerBuiltinSystems
    // In the future, this could load from shared libraries
    ...
    spdlog::warn("[PackageLoader] System {} not found in built-in registry", system.name);
    return false;
}
